...
The `ImageGenerationBackend` interface in `cascade_img.backends.base` is the extension point: when the Flux-via-Fal backend ships in v0.3, the same composer, curation tools, MCP server, and CLI drive it without consumer code changes. Planned backends:

- Flux Pro / Dev / Schnell via Fal, Flux Kontext (instruction-edit), Ideogram — v0.3
- Imagen 3 via Vertex, Recraft V3 — v0.4
- OpenAI `gpt-image-1` / DALL-E 3, Stable Diffusion 3.5 via Stability — v0.5
