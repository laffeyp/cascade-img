...
- [ ] `pytest` passes (run from `packages/python/`)
...
- [ ] Vocabulary mirror in sync: `diff vocabulary/0.1.json packages/python/src/cascade_img/vocabulary/versions/0.1.json` prints nothing
- [ ] If the AGENTS.md orientation block changed: re-ran `python3 tools/render_agent_entrypoints.py` and committed the regenerated entry files (`tools/render_agent_entrypoints.py --check` is clean)
